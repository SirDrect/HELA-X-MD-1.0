...
if (!fs.existsSync(__dirname + '/session/creds.json')) {
    if (!config.SESSION_ID) return console.log("Please Add SESSION_ID ➾")
      const sessdata = config.SESSION_ID.split("WA-BOT=")[1];
...
async function connectToWA() {;
    ...
    conn.ev.on('connection.update', async (update) => {
        ...
        if (connection === 'close') {
            ...
            console.log('WA BOT Plugins installed ✅')
...
conn.sendMessage(conn.user.id,{ text: up, contextInfo: {
        ...
        externalAdReply: { 
          ...
          mediaType: 1,
          sourceUrl: "",
          thumbnailUrl: "https://i.ibb.co/wFkjNGMj/61b1ec5e-37b8-45ff-b65f-9b8bf57461f0.jpg",
...
app.get("/", (req, res) => {
res.send("WA BOT CONNECTED SUCCESSFUL ✅");
});
app.listen(port, () => console.log(`WA BOT Server listening on port http://localhost:` + port));
setTimeout(() => {
connectToWA()
}, 3000);
